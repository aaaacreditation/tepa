...
async function deliver(enquiry: Enquiry) {
  ...
  /* Queue the "lead" conversion for each ad platform in the same request that
     ...
     GOOGLE_ADS_ACTION_LEAD, which is how a setup that already counts the
     submit with the gtag snippet avoids counting it twice. Meta's row needs
     the Conversions API token and carries the pixel's event id, which is how
     its two halves are counted once. */
